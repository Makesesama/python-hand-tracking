"""Shared msgspec struct definitions for hand tracking data."""
import msgspec
...
def leap_event_to_msgspec(event: leap.TrackingEvent) -> TrackingEvent:
    """
    Convert a Leap Motion TrackingEvent to a msgspec TrackingEvent struct.

    Args:
        event: The Leap Motion tracking event

    Returns:
        TrackingEvent msgspec struct ready for serialization
    """
    hands = []
    ...
    for hand in event.hands:
        ...
        for digit in hand.digits:
            ...
            finger_struct = Finger(
                id=digit.finger_id,
                tip_position=Vector3(
                    x=digit.tip_position.x,
                    y=digit.tip_position.y,
                    z=digit.tip_position.z
                ),
        ...
        hand_struct = Hand(
            ...
            palm_velocity=Vector3(
                x=hand.palm.velocity.x,
                y=hand.palm.velocity.y,
                z=hand.palm.velocity.z
            ),
            palm_normal=Vector3(
            ...
            direction=Vector3(
                x=hand.palm.direction.x,
                y=hand.palm.direction.y,
                z=hand.palm.direction.z
            ),
            wrist_position=Vector3(
                x=arm.wrist.x,
                y=arm.wrist.y,
